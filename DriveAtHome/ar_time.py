...
import cv2 ,aruco ,socket ,random,base64,math,numpy as np
...
# obj3  = three_d_object('data/3d_objects/foxme.obj', 'data/3d_objects/texture.png')
...
def ar_score(player):
	...
	while True:
		...
		if ((int(elapsedTime)  == 60)):
			insert_to_score(player, getCurrentTime(), str(score))
		if (int(elapsedTime) == 63):
			break
		...
		timeTest = "Time: " + str(int(abs(elapsedTime-60)))
