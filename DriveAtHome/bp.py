#!/usr/bin/python
import os, sys, time, numpy as np, cv2, math, poseModule as pm
from sys import *
...
steer_front = (0, 255, 0) #green
steer_right = (255, 0, 0) #blue
steer_left = (255, 0, 0) #blue
...
def bp():
    ...
    while True:
        ...
        # right
        if(lmList[16][2] > lmList[0][2] and lmList[15][2] < lmList[0][2]):
            steeringAngle = steering_angle(lmList[27][1], lmList[27][2],
        ...
        # Direction and Stop
        # if both hands up 
        # should use higher than nose
        if (lmList[16][2] < 380 or lmList[15][2] < 380 and selected_direction != 0):
            # Go time
            status = 1
        else:
            ...
            status = 0
            Stop()
            if (380 < lmList[16][2] < 480 and 0 < lmList[16][1] < 160 and lmList[16][2] > 380):
                selected_direction = 1
                print('<-------BACKWARD', status)
            elif (380 < lmList[15][2] < 480 and 480 < lmList[15][1] < 640 and lmList[15][2] > 380):
        ...
        # Gestures detection
        if status == 1:
            #speed value is depend on legs
            if((lmList[28][1] <= 0.0 or lmList[27][1] >= 640.0) or 
            (lmList[28][2]>=380.0 or lmList[27][2] >= 380.0)):
                speed = 0.0
            else:
                speed = int(speed_value(lmList[27][1], lmList[28][1]))
            #if both hand higer than eye than speed + 30
            # if(lmList[15][2] > lmList[4][2]+30 and lmList[16][2] > lmList[4][2]+30):
            #     speed += 30
            if speed < 0:
                speed = 0
            if (min_angle_front < steeringAngle < max_angle_front and lmList[16][2] < 380.0 and lmList[15][2] < 380.0
                                                                    and lmList[27][2] < 380.0 and lmList[28][2] < 380.0):
                ...
                if (max_angle_front < steeringAngle < 90.0 and lmList[16][2] < 380.0 #positive is left
                        and lmList[15][2] < 380.0 and lmList[27][2] < 380.0 and lmList[28][2] < 380.0) :
                    # L+speed
                    ...
                    if (-90.0 < steeringAngle < min_angle_front and lmList[16][2] < 380.0 #negative is right
                            and lmList[15][2] < 380.0 and lmList[27][2] < 380.0 and lmList[28][2] < 380.0):
                        # R+speed
        ...
        #     c = 0
        try:
            if (380 < lmList[15][2] < 480 and 480 < lmList[15][1] < 640 and 380 < lmList[16][2] < 480 and 0 < lmList[16][1] < 160):
                cv2.rectangle(img, (160, 400), (480, 420),
                              (0, 255, 0), thickness=2)
                quit_count = quit_count + 1
                if quit_count > 10:
...
def speed_value(y1, y2):
    average_wirsts = abs((y1 - y2) / 2)
    average_wirsts -= 10
    if average_wirsts < 0:
        average_wirsts = 0
    speed = average_wirsts
